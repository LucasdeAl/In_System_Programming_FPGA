 ...
 *
 * SVN $Revision: 6147 $
 * SVN $Date: 2014-02-19 12:28:26 +0530 (Wed, 19 Feb 2014) $
 */
...
#endif
{
    ...
    /*
     ...
     Hence moveing Disconnect handling out of get_mode condition.
     */
    ...
    g_mss_usb_host_cb.usb_host_disconnect();
#endif //MSS_USB_HOST_ENABLED
    }
...
)
{
    ...
    g_mss_usb_host_cb.usb_host_cep(status);
...
)
{
    ...
    while(irq_num)
    {
        ...
        if((irq_num) && (irq_num & MSS_USB_WORD_BIT_0_MASK))
        {
            if(MSS_USB_HCIF_tx_ep_is_retry_err(ep_num))
            {
                status |= MSS_USB_EP_NO_RESPONSE;
                MSS_USB_HCIF_tx_ep_clr_retry_err(ep_num);
            }
            if(MSS_USB_HCIF_tx_ep_is_rxstall_err(ep_num))
            {
                status |= MSS_USB_EP_STALL_RCVD;
                MSS_USB_HCIF_tx_ep_clr_rxstall_err(ep_num);
            }
            if(MSS_USB_HCIF_tx_ep_is_naktimeout_err(ep_num))
            {
                status |= MSS_USB_EP_NAK_TOUT;
                MSS_USB_HCIF_tx_ep_clr_naktimeout_err(ep_num);
            }
            if(status == 0u)    /*No error was found*/
            {
                status = MSS_USB_EP_TXN_SUCCESS;
            ...
            g_mss_usb_host_cb.usb_host_tx_complete((uint8_t)ep_num, status);
        }
        status = 0u; /*resetting for next EP status*/
        ++ep_num;
    }
...
)
{
            ...
            */
            ...
            if(MSS_USB_CIF_rx_ep_is_rxpktrdy(ep_num))
            {
                ...
                if(DMA_DISABLE == (gd_rx_ep[ep_num]->dma_enable))
                {
                    ...
                    received_count = 0 is also valid condition,
                    meaning zlp received.
                    */
                    if(received_count)
                    {
    ...
    while(irq_num)
    {
        ...
        if((irq_num) && (irq_num & MSS_USB_WORD_BIT_0_MASK))
        {
            if(MSS_USB_HCIF_rx_ep_is_rxstall_err(ep_num))
            {
                status |= MSS_USB_EP_STALL_RCVD;
                MSS_USB_HCIF_rx_ep_clr_rxstall_err(ep_num);
            }
            if(MSS_USB_HCIF_rx_ep_is_naktimeout_err(ep_num))
            {
                status |= MSS_USB_EP_NAK_TOUT;
                MSS_USB_HCIF_rx_ep_clr_naktimeout_err(ep_num);
            }
            if(MSS_USB_HCIF_rx_ep_is_retry_err(ep_num))
            {
                status |= MSS_USB_EP_NO_RESPONSE;
                MSS_USB_HCIF_rx_ep_clr_retry_err(ep_num);
            }
            if(MSS_USB_HCIF_rx_ep_is_rxpktrdy(ep_num))
            {
                status = MSS_USB_EP_TXN_SUCCESS;
            }
            g_mss_usb_host_cb.usb_host_rx((uint8_t)ep_num, status);
        }
        status = 0u; /*resetting for next EP status*/
        ++ep_num;
    }
...
/***************************************************************************//**
 ...
 Occured and corresponding EP number then calls-back to upper layer to indicate
 the event.
 */
#if defined(__GNUC__)
__attribute__((__interrupt__)) void USB_DMA_IRQHandler(void)
            ...
            */
            ...
            if(core_ep->dma_enable == DMA_ENABLE)
            {
                ...
                /*
                 ...
                 TxPktRdy bit here, is not envoking EP interrupt.
