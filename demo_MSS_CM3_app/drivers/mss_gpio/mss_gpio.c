...
/*******************************************************************************
 * MSS_GPIO_toggle_output
 * Toggle the output state of the specified GPIO pin.
 *
 * See "mss_gpio.h" for details of how to use this function.
 */
void MSS_GPIO_toggle_output
(
    mss_gpio_id_t port_id
)
{
    ...
    if(gpio_idx < NB_OF_GPIO)
    {
        uint32_t gpio_setting = GPIO->GPIO_OUT;

        /* Toggle the state of the specified GPIO pin */
        gpio_setting ^= ((uint32_t)0x01u << gpio_idx);

        /* Write the new value back to the GPIO_OUT register */
        GPIO->GPIO_OUT = gpio_setting;
    }
}
